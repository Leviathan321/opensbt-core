...
# The original crossover algorithm from deap is buggy, it yealds nan numbers in cx operation (l. 363/364 is added)
# 
        ...
        if random.random() <= 0.5:
            ...
            if abs(ind1[i] - ind2[i]) > 1e-14:
                ...
                if rand <= 1.0 / alpha:
                    ...
                    # print( f"alpha = { alpha}")
                    # print( f"beta = { beta }")
                    # print( f"beta_q = (1.0 / (2.0 -  {rand * alpha}) ** ({1.0 / (eta + 1)} ")
                    # print( f"beta_q base = {1.0 / (2.0 - rand * alpha) }")
                    # print( f"beta_q potenz = { (1.0 / (eta + 1))}")
                ...
                try:
                    ...
                    assert ( math.isnan(c1) == False)
                    assert ( math.isnan(c2) == False) 
                except AssertionError as e: 
                    print(ind1)
                    print(ind2)
                    print(f"bounds: {low} | {up}")
                    raise e
