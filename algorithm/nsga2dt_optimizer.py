...
# variation_label = str(["NON-DOMINATED" if USE_NDS_FOR_SEARCH else 'ALL',
...
class NsgaIIDTOptimizer(Optimizer):
    ...
    def run(self) -> ResultExtended:
        ...
        if maximal_execution_time is not None:
            ...
            print("Search is constrained by maximal execution time")
        elif max_tree_iterations is not None:
            _maximal_execution_time = sys.maxsize
            print("Search is constrained by maximal number of tree generations")
        else:
            print("Parameters are not correctly set, cannot start search.")
            sys.exit()
        ...
        tree_iteration = 0  # tree iteration counter initialization
        while self.is_time_left_for_next_iteration(start_time,
                                                   _maximal_execution_time, problem.approx_eval_time,
                                                   critical_regions,
                                                   inner_num_gen) and tree_iteration < max_tree_iterations:
            # extend the history by one generation
            hist_holder.extend([inner_algorithm] * inner_num_gen)

            print(f"running iteration {tree_iteration}")
    ...
    def is_time_left_for_next_iteration(self, start_time, _maximal_execution_time, simulation_time, critical_regions,
        ...
        if DO_EVAL_APPROXIMATION:
            ...
            print(f"Time left for next iteration: {time_min}")
            print(f"Time required for next iteration: {time_real}")
    ...
    def is_time_left_for_evaluation(self, start_time, _maximal_execution_time, simulation_time, population):
        if DO_EVAL_APPROXIMATION:
            ...
            print(f"Time left for evaluation of population: {time_min}")
            print(f"Time required for evaluation of population: {time_real}")
            return time_real < time_min + tolerance
        else:
            return (time.time() - start_time) < _maximal_execution_time
